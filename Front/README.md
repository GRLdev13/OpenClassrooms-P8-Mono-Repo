...
To run the Karma test suite and generate a JUnit XML report, use the Python test runner:

```bash
python run-tests.py
```

The script runs the tests in headless Chrome and writes the report to
`test-results/junit.xml`. This file can be uploaded as an artifact or consumed by
a JUnit test reporter in a GitHub Actions CI/CD workflow. The script exits with a
non-zero status when the tests fail or the report cannot be generated.
