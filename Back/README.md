...
Run the test launcher from the repository root with Bash:

```bash
bash ./run-tests.sh
```

The launcher requires Bash, Python 3.8 or newer, and JDK 21.

`run-tests.py` runs the Gradle/JUnit test suite and
copies the generated JUnit XML reports to `test-results/`.
 It clears reports from previous runs first and returns the Gradle test exit code, so it can be
used directly as a CI step. 

Reports generated by failed tests are still copied
for later inspection.

In GitHub Actions, run the script and upload the reports even if a test fails:
...
To run Gradle directly without copying reports to the CI directory, use:

```bash
./gradlew clean test
```

Gradle's original XML reports are generated in `build/test-results/test/`.
